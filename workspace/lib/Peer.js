...
class Peer {
    constructor(socket, userId, room) {
        ...
        this.socket.on('message', (data) => {
            console.log(`lym recv message msg:${JSON.stringify(data)} this.userId:${this.userId}`);
            this.room.handleMessage(this.userId, data);
        });
        this.socket.on('chat', (data) => {
            console.log(`lym recv  chat message msg:${JSON.stringify(data)} this.userId:${this.userId}`);
            this.room.handleChat(this.userId, data);
        });
        this.socket.on('leave', (data) => {
            console.log(`lym recv leave message msg:${JSON.stringify(data)} this.userId:${this.userId}`);
            this.room.handleUserLeave(this.userId, data);
        });
        // 绑定断开连接事件
        this.socket.on('disconnect', () => {
            console.log(`lym recv disconnect message  this.userId:${this.userId}`);
